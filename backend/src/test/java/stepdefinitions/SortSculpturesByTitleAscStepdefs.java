...
public class SortSculpturesByTitleAscStepdefs {
    ...
    public void iHaveASortSculpturesByTitleAscSpringEndpoint() {
        uriBase = "http://localhost:8080";
        restClient = RestClient.create();
    }

    @When("I call the sort sculptures by title asc endpoint")
    public void iCallTheSortSculpturesByTitleAscEndpoint() {
        sculptures = restClient.get()
                .uri(uriBase + "/sculptures/sortAllByTitle/asc")
                .retrieve()
                .body(new ParameterizedTypeReference<List<SculptureDTO>>() {});
    }

    @Then("I should get back a list of sculptures sorted by title asc")
    public void iShouldGetBackAListOfSculpturesSortedByTitleAsc() {
        Assertions.assertNotNull(sculptures);
    }

    @And("each item should returned in ascending title order and contain the {string}, {int} and {string} of the sculptures at a given {int}")
    public void eachItemShouldReturnedInAscendingTitleOrderAndContainTheTitleYear_CompletedAndArtist_NameOfTheSculpturesAtAGivenIndex(String title, int yearCompleted, String artistName, int index) {
